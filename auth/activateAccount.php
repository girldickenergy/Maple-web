<?php
    ...
    if (isset($_GET["hash"]) && !empty($_GET["hash"]))
    {
        ...
        if ($user != null && $user["UniqueHash"] == $_GET["hash"] && ($user["Permissions"] & perm_activated) == 0 && SetPermissions($user["ID"], $user["Permissions"] + perm_activated))
        {
            $failed = false;
...
<html>
    <head>
        ...
        <title><?= GetLocalizedString("TITLE_ACCOUNT_ACTIVATION").'- Maple' ?></title>
    ...
    <body>
        ...
        <div class="full-height-container d-flex flex-column justify-content-center align-items-center text-center" data-aos="fade" data-aos-duration="1000" data-aos-once="true">
            <h1 class="fw-bold"><?= GetLocalizedString("AUTH_ACCOUNT_ACTIVATION"); ?></h1>
            <?php
            if (!$failed)
            {
                echo(GetLocalizedString("AUTH_ACCOUNT_ACTIVATION_SUCCESS"));
            }
            else
            {
                echo(GetLocalizedString("AUTH_ACCOUNT_ACTIVATION_FAIL"));
            }
            ?>
        </div>
