...
<html>
    <head>
        ...
        <title><?= GetLocalizedString("TITLE_PASSWORD_RECOVERY").'- Maple' ?></title>
    ...
    <body>
        ...
        <div class="full-height-container d-flex flex-column justify-content-center align-items-center text-center" data-aos="fade" data-aos-duration="1000" data-aos-once="true">
            <h1 class="fw-bold"><?= GetLocalizedString("AUTH_PASSWORD_RECOVERY"); ?></h1>
            <?php
                if ($status == 0 && $completed)
                {
                    echo(GetLocalizedString("AUTH_PASSWORD_RECOVERY_SUCCESS"));
                }
                else
                {
                    echo('<div class="auth-form mt-4">
                              <div class="p-4 text-start">
                                  <form action="'.$self.'" method="post">
                                      <div>
                                          <p class="m-0">'.GetLocalizedString("AUTH_PASSWORD_RECOVERY_EMAIL").'</p>
                                          <input type="text" name="email" class="form-control" required>
                                          <p class="m-0 text-danger" '.($status != 0  ? '' : 'hidden').'>'.($status == 1 ? GetLocalizedString("AUTH_PASSWORD_RECOVERY_ACCOUNT_NOT_FOUND") : GetLocalizedString("AUTH_PASSWORD_RECOVERY_UNKNOWN_ERROR")).'</p>
                                      </div>

                                      <div class="mt-3">
                                          <button type="submit" name="submit" class="btn btn-primary w-100">'.GetLocalizedString("AUTH_PASSWORD_RECOVERY_RECOVER").'</button>
                                      </div>
                                  </form>
                              </div>
                          </div>');
